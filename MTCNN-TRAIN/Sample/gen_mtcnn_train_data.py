...
d_idx = 0 # dont care
item_id = 0 # 数据库的id
batch_size = 20 #多少图片进行一次写入,防止缓存不足

# create the lmdb file
# map_size指的是数据库的最大容量，根据需求设置
lmdb_env_12 = lmdb.open('mtcnn_train_12', map_size=100000000)
lmdb_txn_12 = lmdb_env_12.begin(write=True)
lmdb_env_24 = lmdb.open('mtcnn_train_24', map_size=1000000000)
lmdb_txn_24 = lmdb_env_24.begin(write=True)
lmdb_env_48 = lmdb.open('mtcnn_train_48', map_size=1000000000)
...
for line_idx,annotation in enumerate(annotations):
    ...
    for box in boxes:
        ...
        range_num = boxes_num * 10;
        
        
        for i in range(range_num):
            print "%s images done, pos: %s part: %s neg: %s"%(line_idx + 1, p_idx, d_idx, n_idx)
            ...
            elif IoU(crop_box, box_) >= 0.4 and (part_num < pos_num*1.25) :
    ...
    while neg_num < 3 * pos_num:
        ...
        if np.max(Iou) < 0.3:
            ...
            ''' size 12'''
            mtcnn_datum = data_tran_tool.array_to_mtcnndatum(resized_im12, 0, [offset_x1, offset_y1, offset_x2, offset_y2])
            keystr = '{:0>8d}'.format(item_id)
            lmdb_txn_12.put(keystr, mtcnn_datum.SerializeToString())
            
            
            mtcnn_datum = data_tran_tool.array_to_mtcnndatum(resized_im24, 0, [offset_x1, offset_y1, offset_x2, offset_y2])
            keystr = '{:0>8d}'.format(item_id)
            lmdb_txn_24.put(keystr, mtcnn_datum.SerializeToString())
           
          
            mtcnn_datum = data_tran_tool.array_to_mtcnndatum(resized_im48, 0, [offset_x1, offset_y1, offset_x2, offset_y2])
            keystr = '{:0>8d}'.format(item_id)
            lmdb_txn_48.put(keystr, mtcnn_datum.SerializeToString())
